...
fun similar_names (subs, {first=f, middle=m, last=z}) =
   let
      val first_name_subs = get_substitutions2 (subs, f) 
      fun generate_alt_full_names subs =
         case subs of
              [] => []
            | s::subs' => {first=s, middle=m, last=z} :: generate_alt_full_names (subs')
   in
      {first=f, middle=m, last=z} :: generate_alt_full_names (first_name_subs)
   end
